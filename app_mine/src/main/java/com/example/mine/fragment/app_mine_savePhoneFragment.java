...
/**
 * @Author winiymissl
 * @Date 2024-02-20 19:59
 * @Version 1.0
 */
public class app_mine_savePhoneFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        String newPhoneNumber = bundle.getString("phone");
        binding.buttonGetCode.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                MyRetrofit.serviceAPI.getCode(new CodeRequest(newPhoneNumber)).enqueue(new Callback<CodeResult>() {
                    @Override
        ...
        binding.buttonSubmit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (binding.inputTextCode.getText().toString() != "") {
                    MyRetrofit.serviceAPI.changePhoneNumber("Bearer "+MMKV.defaultMMKV().getString("token", null), newPhoneNumber, binding.inputTextCode.getText().toString()).enqueue(new Callback<ChangePhoneResult>() {
                        @Override
                        public void onResponse(Call<ChangePhoneResult> call, Response<ChangePhoneResult> response) {
                            if (response.isSuccessful()) {
                                Toast.makeText(getActivity(), "修改成功", Toast.LENGTH_SHORT).show();
                            }
                            Toast.makeText(getActivity(), "修改有问题", Toast.LENGTH_SHORT).show();
                        }

                        @Override
                        public void onFailure(Call<ChangePhoneResult> call, Throwable t) {

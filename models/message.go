...
func sendMsg(userId int64, msg []byte) {
	...
	r, err := dao.RDB.Get(ctx, "online_"+userIdStr).Result()
	if err != nil {
		fmt.Println(err)
	}
	if r != "" {
	...
	res, err := dao.RDB.ZRevRange(ctx, key, 0, -1).Result()
	if err != nil {
		fmt.Println(err)
	}
	score := float64(cap(res)) + 1
	ress, e := dao.RDB.ZAdd(ctx, key, &redis.Z{score, msg}).Result() //jsonMsg
...
func RedisMsg(userIdA int64, userIdB int64, start int64, end int64, isRev bool) []string {
	...
	var err error
	if isRev {
		rels, err = dao.RDB.ZRange(ctx, key, start, end).Result()
	} else {
		rels, err = dao.RDB.ZRevRange(ctx, key, start, end).Result()
	}
	if err != nil {
